define(function(require, exports, module){
	...
	exports.init = function($page, tmplData, criteriaData, columnData, module){
		...
		$('#save', $page).click(function(){
			...
			if(tmplTitle){
				...
				require('ajax').postJson('admin/tmpl/stat/ltmpl/save', {
					...
					if(data.status === 'suc'){
						...
						var tpage = require('page').getPage( module + '_stat_ltmpl_list');
						if(tpage){
							tpage.refresh();
						}
					}else{
		...
		function getCriteriaData(){
			...
			$('.criterias-container', $page).children('.criteria-item').each(function(){
				...
				if(criteria.isFieldAvailable()){
					...
					$.extend(itemData, {
						relation		: 'and',
						filterOccasion	: criteria.getFilterOccasion(),
	...
	function initCriteria($page, criteriaData, module){
		...
		$('#filter-occasion', $page).change(function(){
			var filterOccasion = $(this).val();
			handleSelectedItem(function(){
				this.setFilterOccasion(filterOccasion);
			});
		});
	...
	function Criteria(_param, $page){
		...
		this.initFromData = function(data){
			if(data){
				...
				this.setFilterOccasion(data.filterOccasion);
		...
		this.getFilterOccasion = function(){
			return param.filterOccasion;
		};
		
		this.setFilterOccasion = function(filterOccasion){
			param.filterOccasion = filterOccasion;
			callbackMap.fire('aftersetFilterOccasion', [filterOccasion]);
		};
