...
<div id="${module.name }-statview-list-${RES_STAMP}" class="detail module-list-tmpl statview-list">
	<div class="page-header">
		...
		<div class="header-buttons">
			...
			<a class="" title="重新统计" id="btn-recalc" confirm="确认重新统计？" href="admin/stat/view/recalc/${menu.id }">
				<i class="fa fa-coffee"></i>
			</a>
			<a class="hbtn-purple" title="显示列" id="btn-viewcols" href="page:#viewcols-window.toggle">
	...
	<div class="page-body">
		<form class="form-inline"  action="admin/stat/view/index/${menu.id }">
			<input type="hidden" id="disabledColIds" name="disabledColIds" value="${disabledColIds }" />
						...
						<c:if test="${criteriaItem.fieldAvailable }">
							<c:choose>
								<c:when test="${criteriaItem.inputType == 'text' }">
									<input class="form-control" type="text" name="criteria_${criteriaItem.id }" value="${criteria.reqCriteriaMap[criteriaItem.id]}" placeholder="${criteriaItem.placeholder }" />
								</c:when>
								<c:when test="${criteriaItem.inputType == 'select' }">
									<select class="form-control cpf-select2" name="criteria_${criteriaItem.id }" data-value="${criteria.reqCriteriaMap[criteriaItem.id]}">
								...
								<c:when test="${criteriaItem.inputType == 'multiselect' }">
									<select class="form-control cpf-select2 format-submit-value" name="criteria_${criteriaItem.id }" multiple="multiple" data-value="${criteria.reqCriteriaMap[criteriaItem.id]}">
										<c:forEach var="option" items="${view.criteriaOptionMap[criteriaItem.fieldId]}">
											<option value="${option.value }">${option.title}</option>
										</c:forEach>								
									</select>
								...
								<c:when test="${criteriaItem.inputType == 'date' }">
									<input class="form-control datepicker" autocomplete="off" type="text" name="criteria_${criteriaItem.id }" value="${criteria.reqCriteriaMap[criteriaItem.id]}"  />
								</c:when>
								<c:when test="${criteriaItem.inputType == 'label' }">
									<span class="cpf-select2-container">
										<select class="cpf-select2 format-submit-value" name="criteria_${criteriaItem.id }" multiple="multiple" data-value="${criteria.reqCriteriaMap[criteriaItem.id]}">
								...
								<c:when test="${criteriaItem.inputType == 'datetime' }">
									<input class="form-control datetimepicker" autocomplete="off" type="text" name="criteria_${criteriaItem.id }" value="${criteria.reqCriteriaMap[criteriaItem.id]}"  />
								</c:when>
								<c:when test="${criteriaItem.inputType == 'time' }">
									<input class="form-control timepicker" autocomplete="off" type="text" name="criteria_${criteriaItem.id }" value="${criteria.reqCriteriaMap[criteriaItem.id]}"  />
								</c:when>
								<c:when test="${criteriaItem.inputType == 'yearmonth' }">
									<input class="form-control yearmonthpicker" autocomplete="off" type="text" name="criteria_${criteriaItem.id }" value="${criteria.reqCriteriaMap[criteriaItem.id]}"  />
								</c:when>
								<c:when test="${criteriaItem.inputType == 'ymrange' }">
									<input class="form-control ymrangepicker" autocomplete="off" type="text" name="criteria_${criteriaItem.id }" value="${criteria.reqCriteriaMap[criteriaItem.id]}"  />
								</c:when>
								<c:when test="${criteriaItem.inputType == 'decimal' }">
									<input class="form-control cpf-field-decimal" autocomplete="off" type="text" name="criteria_${criteriaItem.id }" value="${criteria.reqCriteriaMap[criteriaItem.id]}"  />
								</c:when>
								<c:when test="${criteriaItem.inputType == 'int' }">
									<input class="form-control cpf-field-int" autocomplete="off" type="text" name="criteria_${criteriaItem.id }" value="${criteria.reqCriteriaMap[criteriaItem.id]}"  />
								</c:when>
								<c:otherwise>
				...
				<tbody>
					<c:forEach items="${view.parsers }" var="parser" varStatus="i">
						<tr data-code="${parser.code }">
							<c:forEach items="${view.enabledColumns }" var="column" varStatus="j" >
